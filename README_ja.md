...
- 表示中ファイル内検索
- ライブリロード — Markdown の追加・削除・リネーム・更新を検知してツリーとプレビューを更新
- サイドバー位置・表示状態・幅・テーマ・フォントサイズ・最近開いたフォルダ・最後に開いたファイル・ツリー開閉状態を保存
- ローカル動作 — テレメトリなし、Markdown 本文を外部送信しない
...
| **Core** | `src/SkimDown/Core/` | フォルダ権限、file bookmark、設定保存、ファイル監視 |
...
make run-cli            # このチェックアウト用の一時 skimdown コマンドで起動
make launch-check       # GUI スモークテスト（ビルド・起動・ウィンドウ確認）
make cli-launch-check   # ローカルビルドの CLI symlink スモークテスト
...
`make run-cli SKIMDOWN_TARGET=/path/to/folder-or-file.md` を使うと、`/Applications/SkimDown.app` や `/usr/local/bin/skimdown` を置き換えずに、ローカルのデバッグビルドを一時 `skimdown` コマンド経由で確認できます。
